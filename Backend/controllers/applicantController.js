...
exports.getApplicantById = async (req, res, next) => {
    ...
    try {
      const [applicant, _] = await pool.execute("CALL get_applicant_by_id(?)", [id]);
...
exports.createNewApplicant = async (req, res, next) => {
    const { first_name,last_name,phone_number,city,email,applicant_password } = req.body;
    try {
      const applicant = await pool.execute("CALL add_new_applicant(?,?,?,?,?,?)",
...
exports.updateApplicantById = async (req, res, next) => {
    ...
    const { first_name,last_name,phone_number,city,email,applicant_password } = req.body;
      try {
        const applicant = await pool.execute("CALL update_applicant(?,?,?,?,?,?,?)",
